import { storage, firestore } from './firebase'; // Import Firebase configuration
...
const uploadImages = async () => {
  ...
  try {
    for (const [index, imageUrl] of images.entries()) {
      ...
      const storageRef = storage.ref().child(`images/image${index + 1}.jpg`);
      ...
      console.log(`Uploaded image ${index + 1} and saved URL to Firestore`);
    }
  } catch (error) {
